...
class SQLTable(T)
{
    ...
    this(LibsqlClient client,string table_name)
    {
        this.table_name = table_name;
        this.client=client;
    }

    private void create_table_internal(string sql_inicial)
    {
        ...
        foreach (member; __traits(allMembers, T))
        {
            ...
            static if (hasUDA!(__traits(getMember, T, member), SQL) && (prot == "public"))
            {
                ...
                else //else static if (is(typeof(__traits(getMember, T, member))==string))
                {
                    sql ~= " TEXT NOT NULL ";
                }
                col++;
    ...
    void create(T)()
    {
        ...
        create_table_internal!T(table_name, sql);
    }

    void create_if_not_exists()
    {
        string sql = "CREATE TABLE IF NOT EXISTS " ~ table_name ~ "(";
        create_table_internal(sql);
    }
   ...
   void insert(T p)
    {
        ...
        debug writeln("insert", p, " into ", table_name);
        ...
        if (retval != 0)
            throw new Exception("libsql_prepare error:" ~ to!string(err));
        ...
        foreach (i, member; __traits(allMembers, T))
        {
            ...
            static if (hasUDA!(__traits(getMember, T, member), SQL) && (prot == "public"))
            {
                ...
                static if (__traits(isIntegral, typeof(__traits(getMember, p, member))))
                {
                    ...
                    retval = libsql_bind_int(insert_stmt, i + 1,
                            to!long(__traits(getMember, p, member)), &err);
                }
                else static if (is(typeof(__traits(getMember, p, member)) == double))
                {
                    retval = libsql_bind_float(insert_stmt, i + 1,
                            __traits(getMember, p, member), &err);
                    debug writeln("orm: ", member, "->FLOAT");
                }
                else static if (is(typeof(__traits(getMember, p,
                        member)) == Date) || is(typeof(__traits(getMember, p, member)) == DateTime))
                {
                    retval = libsql_bind_string(insert_stmt, i + 1,
                            toStringz(__traits(getMember, p, member).toISOExtString()), &err);
                    debug writeln("orm: ", member, "-> ExtString");
                }
                else //static if (is(typeof(__traits(getMember, p, member))==string))
                {
                    retval = libsql_bind_string(insert_stmt, i + 1,
                            toStringz(to!string(__traits(getMember, p, member))), &err);
                    debug writeln("orm: ", member, "->STRING");
                }
                if (retval != 0)
                    throw new Exception("insert:" ~ to!string(err));
            }
        }

        retval = libsql_execute_stmt(insert_stmt, &err);
        if (retval != 0)
            throw new Exception("libsql_execute_stmt:" ~ to!string(err));
...
class LibsqlClient
{
    ...
    void connect(string url, string auth_token, bool print_sql = false)
    {
        ...
        if (retval != 0)
            throw new Exception("libsql_open_any error:" ~ to!string(err));

        retval = libsql_connect(db, &conn, &err);
        if (retval != 0)
            throw new Exception("libsql_connect error:" ~ to!string(err));
    }
        ...
        foreach (i, member; __traits(allMembers, T))
        {
            ...
            static if (hasUDA!(__traits(getMember, T, member), SQL) && (prot == "public"))
            {
                ...
                static if (__traits(isIntegral, typeof(__traits(getMember, T, member))))
                {
                    ...
                    if (retval != 0)
                        throw new Exception("libsql_get_int:" ~ to!string(err));
                    __traits(getMember, result, member) = cast(typeof(__traits(getMember,
                            T, member))) value;
                }
                else static if (is(typeof(__traits(getMember, T, member)) == double))
                {
                    ...
                    if (retval != 0)
                        throw new Exception("libsql_get_float:" ~ to!string(err));
                    __traits(getMember, result, member) = cast(typeof(__traits(getMember,
                            T, member))) value;
                }
            ...
            else static if (is(typeof(__traits(getMember, T, member)) == string))
                {
                    ...
                    if (retval != 0)
                        throw new Exception("libsql_get_string:" ~ to!string(err));
                ...
                else static if (is(typeof(__traits(getMember, T, member)) == DateTime))
                {
                    ...
                    if (retval != 0)
                        throw new Exception("libsql_get_string:" ~ to!string(err));
                    __traits(getMember, result, member) = DateTime.fromISOExtString(
                            to!string(value));
                }
                col++;
            }
